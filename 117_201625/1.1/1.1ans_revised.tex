...
\includegraphics[scale=0.5]{start.pdf}
{{{1\linewidth}{Let $f(x)=x^5-7x+3$.  $f(x)$ is always continuous because it is a polynomial.  Thus, since $f(x)$ is continuous on $[1,3]$ with $f(1)=-3<0$ and $f(2)=21>0$, the intermediate value theorem guarantees at least one solution to $f(x)=0$ on  the interval $(1,2)$.}}}
\includegraphics[scale=0.5]{end.pdf}
